...
class Config(object):
    LOG_DIRECTORY = '/var/log/hed2tools'
    LOG_FILE = os.path.join(LOG_DIRECTORY, 'error.log')
    if not os.path.exists('/var/log/hed2tools/tmp.txt'):
        f = open('/var/log/hed2tools/tmp.txt', 'w+')
        f.write(str(os.urandom(24)))
        f.close()
    f = open('/var/log/hed2tools/tmp.txt', 'r')
    SECRET_KEY = f.read()  # os.getenv('SECRET_KEY') # os.urandom(24)
    f.close()
    STATIC_URL_PATH = None
    STATIC_URL_PATH_ATTRIBUTE_NAME = 'STATIC_URL_PATH'
    UPLOAD_FOLDER = os.path.join(tempfile.gettempdir(), 'hed2tools_uploads')
    URL_PREFIX = None
...
class ProductionConfig(Config):
    ...
    TESTING = False
    URL_PREFIX = '/hed2'
    STATIC_URL_PATH = '/hed2/hedweb/static'
